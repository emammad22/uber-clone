        ...
        {onboarding?.map((item, index) => {
          return (
            <View key={item.id} className="flex justify-center items-center p-5">
              ...
              <View className="flex justify-center items-center w-full mt-10 flex-row">
                <Text className="font-bold text-black text-3xl mx-10 text-center">{item.title}</Text>
              </View>
              <Text className="text-lg font-JakartaSemiBold text-center test-[#858585] mx-10 mt-3">
                {item.description}
              </Text>
            </View>
          );
